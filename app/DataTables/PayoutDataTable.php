...
class PayoutDataTable extends DataTable
{
    ...
    public function query(Trips $model)
    {
        ...
        if(LOGIN_USER_TYPE=='company') {
                $trips->where('users.company_id',Auth::guard('company')->user()->id);
        }else{
                $trips->where('users.company_id',1);
        }

        if($this->filter_type == 'Weekly') {
            $trips = $trips->where('driver_id',$driver_id)->groupBy(DB::raw('WEEK(trips.created_at,1)'));
        }
    ...
    protected function getColumns()
    {
        if($this->filter_type == 'Weekly') {
            return array(
                ['data' => 'driver_id', 'name' => 'users.id', 'title' => 'Driver Id'],
