...
class CompanyOweDataTable extends DataTable
{
    ...
    public function query()
    {
        ...
        $owe = DB::table('users')
            ...
            ->select(['users.id As id', 'users.first_name', DB::raw("GROUP_CONCAT(DISTINCT trips.id) as trip_ids"),
                DB::raw(' concat("' . $this->currency_symbol . '", sum(trips.owe_amount)) as owe_amount'),
                DB::raw(' concat("' . $this->currency_symbol . '", driver_owe_amounts.amount) as remaining_owe_amount '),
                DB::raw(' concat("' . $this->currency_symbol . '",(SELECT sum(driver_owe_amount_payments.amount) FROM driver_owe_amount_payments WHERE user_id = users.id)) as paid_amount')
            ])
